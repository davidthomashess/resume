...
const ColorPicker = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (hexColor.toString().match(hexHash)) {
        const redHex = hexColor
          .toString()
          .slice(1)
          .match(/.{1,2}/g)[0];
        const grnHex = hexColor
          .toString()
          .slice(1)
          .match(/.{1,2}/g)[1];
        const bluHex = hexColor
          .toString()
          .slice(1)
          .match(/.{1,2}/g)[2];
  ...
  const redText = (val) => {
    ...
    setHexes((previousHexes) =>
      Object.assign([], previousHexes, { 0: num <= 255 ? hex : "ff" })
    );
    setRgb((previousRgb) =>
      Object.assign([], previousRgb, { 0: num <= 255 ? num : 255 })
    );
  };
  ...
  const greenText = (val) => {
    ...
    setHexes((previousHexes) =>
      Object.assign([], previousHexes, { 1: num <= 255 ? hex : "ff" })
    );
    setRgb((previousRgb) =>
      Object.assign([], previousRgb, { 1: num <= 255 ? num : 255 })
    );
  };
  ...
  const blueText = (val) => {
    ...
    setHexes((previousHexes) =>
      Object.assign([], previousHexes, { 2: num <= 255 ? hex : "ff" })
    );
    setRgb((previousRgb) =>
      Object.assign([], previousRgb, { 2: num <= 255 ? num : 255 })
    );
  };

  return (
    <div className="mobile-contains-colorPicker">
      <div id="theDisplay" className="mobileColorDisplay"></div>
      <div className="mobile-slidecontainer">
        <div className="mobile-rowItem">
          <input
            className="mobile-slider mobile-slider-red"
            type="range"
            min="0"
            max="255"
            value={rgb[0]}
      ...
      <div className="mobile-slidecontainer">
        <div className="mobile-rowItem">
          <input
            className="mobile-slider mobile-slider-green"
            type="range"
            min="0"
            max="255"
            value={rgb[1]}
      ...
      <div className="mobile-slidecontainer">
        <div className="mobile-rowItem">
          <input
            className="mobile-slider mobile-slider-blue"
            type="range"
            min="0"
            max="255"
            value={rgb[2]}
      ...
      <div>
        ...
        />
        <input
          className="mobile-cp-button"
          type="button"
          onClick={() =>
            navigator.clipboard.writeText(`#${hexes[0]}${hexes[1]}${hexes[2]}`)
          }
          value="copy"
        />
      </div>
